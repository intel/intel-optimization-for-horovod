...
def train(epoch):
    ...
    with tqdm(total=len(train_loader),
        ...
        for batch_idx, (data, target) in enumerate(train_loader):
            ...
            t.update(1)
            if args.iter and batch_idx == args.warm + args.iter: break
...
if __name__ == '__main__':
    ...
    if args.dummy:
        ...
        train_dataset = datasets.FakeData(1281167, (3, 224, 224), 1000,
                                          transforms.ToTensor())
        val_dataset_size = args.num_iterations * args.batch_size if (
            args.dummy and args.num_iterations) else 50000
        val_dataset = datasets.FakeData(val_dataset_size, (3, 224, 224), 1000,
                                        transforms.ToTensor())
    else:
        train_dataset = \
    ...
    for epoch in range(resume_from_epoch, args.epochs):
        train(epoch)
        if args.iter is not None: break
        validate(epoch)
